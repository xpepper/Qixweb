...
public class TestLightInternalIteratorCreationOnTwoCollections extends TestCase
{
	...
	{
		LightInternalIterator iterator = LightInternalIterator.createOn
			(
				new Object[] {"ugl", "vaa", "dip" }, 
				new Object[] {"blocks", "utilities"}
			);
		Object[][] pairElements = (Object[][])iterator.select(new AlwaysTruePredicate(), Object[].class);
		assertEquals("There should be six pairs", 6, pairElements.length);
		
		Pair[] expectedPairs = new Pair[]
		{
			new Pair("ugl","blocks"), new Pair("ugl","utilities"),
			new Pair("vaa","blocks"), new Pair("vaa","utilities"),
			new Pair("dip","blocks"), new Pair("dip","utilities")
		};
