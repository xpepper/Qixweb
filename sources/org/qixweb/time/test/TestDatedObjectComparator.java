...
public class TestDatedObjectComparator extends TestCase
{
    ...
    public void testConsiderDay()
    {
        ...
        assertEquals("DatedObjectComparator must delegate the comparison to CalendarDateComparator", calendarDateComparator.compare(date, dayAfter), datedObjectComparator.compare(
                new SampleDateComparable(date), new SampleDateComparable(dayAfter)));
        assertEquals("DatedObjectComparator must delegate the comparison to CalendarDateComparator", calendarDateComparator.compare(dayAfter, date), datedObjectComparator.compare(
                new SampleDateComparable(dayAfter), new SampleDateComparable(date)));
    }
